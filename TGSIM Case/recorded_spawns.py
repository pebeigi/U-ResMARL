"""Recorded initial traffic cohorts in the TGSIM Foggy Bottom coordinate frame.
...
def catalogue():
    ...
    joined = joined[joined.keep_ego & joined.split.notna() & (joined.tick % 10 == 0)
                    & (joined.speed > .15)]
    eligible_ids = set(joined.id)
    tracks = {int(i): g for i, g in frame.groupby("id") if int(i) in eligible_ids}
...
class RecordedSpawnPool:
    def __init__(self, corridor, sim):
        ...
        for vehicle_id, track in tracks.items():
            ...
            for index in range(len(xy) - 1, 0, -1):
                ...
                if footprint_clearance(corridor, xy[index], heading, length, width) >= margin:
                    ...
                    self.goal_times[vehicle_id] = float(track.time.iloc[index])
                    break
    ...
    def sample(self, env):
        ...
        for _ in range(200):
            ...
            for row in group.iloc[rng.permutation(len(group))].itertuples():
                ...
                if (goal is None or row.time >= self.goal_times[int(row.id)]
                        or np.linalg.norm(goal - pos) < max(5.0, 2 * sim["destination_threshold"])
                        or row.speed > sim["max_agent_speed"]):
                    continue
