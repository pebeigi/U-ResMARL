"""Foggy Bottom roadway: extracted curb polygon, not PCA lanes."""
...
class SiteRoute:
    ...
    def __init__(self, site, start, dest):
        ...
        stations = np.r_[0., np.cumsum(distance)]
        tangent = segment / distance[:, None]
        normals = np.c_[-tangent[:, 1], tangent[:, 0]]
        normals = np.vstack([normals, normals[-1]])
        width = np.array([max(site.clearances(p)[0], .1) for p in points])
        self.geometry = HighwayCorridor(0, 0, points, points-normals*width[:, None],
            points+normals*width[:, None], stations, tangent)
    ...
    def clearances(self, point): return self.site.clearances(point)
    def inside(self, point, margin=0.): return self.site.inside(point, margin)
    def edge_points_at(self, i, t): return self.geometry.edge_points_at(i, t)
    def wall_contacts(self, point): return self.site.wall_contacts(point)
    ...
    def make_local_frame(self, *_args, **_kwargs):
        ...
        width = np.array([max(site.clearances(p)[0], .1) for p in self.center])
...
class SiteNetworkCorridor:
    ...
    def __init__(self, roadway):
        ...
        self.on_road = OnRoadDestField(roadway)  # Frozen calibration geometry.
        self.map_boundaries = [np.asarray(roadway.exterior.coords)] + [np.asarray(r.coords) for r in roadway.interiors]
        # These arrays supply bounds only; network projection requires a route.
        ...
        self.routing_region = roadway.buffer(-3.2).simplify(.15, preserve_topology=True).buffer(-.05, join_style=2)
        if self.routing_region.geom_type != 'Polygon':
            raise ValueError('TGSIM route clearance disconnected the network')
        ...
        self._goal_cache = {}
        self._interior_pool = self._build_interior_pool(clearance=3.5)
    ...
    def _goal_distances(self, dest):
        ...
        if key not in self._goal_cache:
            ...
            if len(self._goal_cache) > 512: self._goal_cache.clear()
    ...
    def route_points(self, start, dest):
        ...
        # Use clearance-aware routes, then physical curb connectors for recorded
        # starts near an edge. Every segment remains inside the actual road.
        ...
        if region.covers(LineString([start, dest])):
            ...
            if not np.isfinite(a).any(): a = self._connections(start)
            if not np.isfinite(b).any(): b = self._connections(dest)
            costs = a[:, None]+self.distances+b[None]
            first, last = np.unravel_index(np.argmin(costs), costs.shape)
            if not np.isfinite(costs[first, last]): raise ValueError('No road-contained route')
            ids = [last]
            while ids[-1] != first:
                previous = int(self.predecessors[first, ids[-1]])
                if previous < 0: raise ValueError('Disconnected route graph')
                ids.append(previous)
            corners = np.vstack([start, self.nodes[ids[::-1]], dest])
        # Densify so closest-segment refinement and local frames are well behaved.
        points = [corners[0]]
        for a, b in zip(corners[:-1], corners[1:]):
            length = np.linalg.norm(b-a)
            if length < 1e-7: continue
            points.extend(a+(b-a)*t for t in np.linspace(0., 1., max(1, int(np.ceil(length/2.)))+1)[1:])
    ...
    def clearances(self, point):
        ...
        return (d if inside else -d), (d if inside else -d), 0.
    ...
    def _build_interior_pool(self, n: int = 2500, clearance: float = 1.2) -> np.ndarray:
        ...
        for _ in range(n * 40):
            ...
            if self.inside(xy, margin=clearance):
                ...
                if len(pts) >= n:
                    break
        if len(pts) < 32:
            raise RuntimeError("TGSIM curb polygon has too little interior for spawn")
    ...
    def sample_interior(self, rng: np.random.Generator, clearance: float, tries: int = 800):
        ...
        for _ in range(int(tries)):
            ...
            if self.inside(xy, margin=float(clearance)):
                return xy
        raise RuntimeError("No interior sample inside the TGSIM curb polygon")
...
def load_site_corridor(csv_path: str | None = None) -> SiteNetworkCorridor:
    ...
    if roadway is None or roadway.is_empty:
        raise FileNotFoundError(f"TGSIM curb polygon missing or empty: {path}")
...
def road_region_from_site(corridor, margin: float = 0.0):
    ...
    if roadway is None:
        raise TypeError("TGSIM road_region requires a site corridor")
