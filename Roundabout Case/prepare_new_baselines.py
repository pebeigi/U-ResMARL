"""Build Jounieh recorded-scene caches for CtRL-Sim / CTG++ (no PCA lanes)."""
    ...
    *,
    ...
    for vehicle_id, track in sorted(tracks.items()):
        ...
        if not np.isfinite(history[-1]).all() or not np.isfinite(history).all():
            ...
            continue
        if not corridor.inside(history[-1], margin=0.4):
        ...
        for t, point in enumerate(on_road_future):
            if np.isfinite(point).all() and not corridor.inside(point, margin=0.8):
                on_road_future[t] = np.nan
        dest = _last_finite(on_road_future)
        if dest is None:
            dest = history[-1].copy()
        if float(np.linalg.norm(dest - history[-1])) < 3.0:
...
def build_recorded_scenes(
    ...
    *,
    ...
    if len(scenes) != count:
        raise ValueError(
            f"Only {len(scenes)} eligible roundabout scenes for {count} requested; "
    ...
    manifest = {
        ...
        "holdout_note": "Jounieh roundabout curb recorded scenes; lanes unused.",
...
def prepare_new_baselines(output_dir: Path, csv_path: Path, cfg, *, train_scenes: int, val_scenes: int, seed: int = 0):
    from new_baselines.data import scene_arrays
    from config import BASE_DESIRED_SPEED

    output_dir = Path(output_dir)
    output_dir.mkdir(parents=True, exist_ok=True)
    future_steps = cfg.context + cfg.horizon + 1
    for split, count in (("train", train_scenes), ("validation", val_scenes)):
        scenes, manifest = build_recorded_scenes(
        ...
        print(f"{split}: {len(scenes)} roundabout recorded scenes -> {path}", flush=True)
    return output_dir


def generation_config():
    from config import MAX_AGENT_SPEED, PERCEPTION_RADIUS, VEHICLE_LENGTH, VEHICLE_WIDTH, VEHICLE_WHEELBASE
    ...
    return Config(
        ...
        steer_from_rest=True,
        perception_radius=PERCEPTION_RADIUS,
    ).validate()


if __name__ == "__main__":
    from config import NEW_BASELINE_DATA, NEW_BASELINE_TRAIN_SCENES, NEW_BASELINE_VAL_SCENES, TRAJECTORIES_CSV

    prepare_new_baselines(
        NEW_BASELINE_DATA,
        TRAJECTORIES_CSV,
        generation_config(),
        train_scenes=NEW_BASELINE_TRAIN_SCENES,
        val_scenes=NEW_BASELINE_VAL_SCENES,
    )
