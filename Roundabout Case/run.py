"""Fresh, budget-matched Jounieh roundabout training, selection and evaluation."""
...
STEMS = dict(
    residual_marl="residual_policy",
    residual_param="residual_param_policy",
    direct_discrete_rl="direct_discrete_policy",
    mappo="mappo_policy",
)
...
def main():
    ...
    os.environ["ROUNDABOUT_RUN_DIR"] = str(run)
    import config as c
    ...
    manifest = dict(
        ...
        run_dir=str(run),
        seeds=c.SEEDS,
        update_limit=c.TRAIN_UPDATES,
        validation_episodes=c.VAL_EPISODES,
        validation_every_updates=c.VAL_EVERY,
        test_scenarios=c.BENCH_SCENARIOS,
        agents=c.NUM_AGENTS,
        max_steps=c.MAX_STEPS,
        perception_radius_m=c.PERCEPTION_RADIUS,
        offline_optimizer_steps=c.NEW_BASELINE_STEPS,
        calibration=str(c.CALIBRATION),
        curb=str(c.STREET_BOUNDARIES),
        vehicle_length_m=c.VEHICLE_LENGTH,
    ...
    def launch(key, command, gpu=None):
        ...
        env = os.environ.copy()
        env["ROUNDABOUT_TORCH_THREADS"] = "1"
        if gpu is not None:
        ...
        with logfile.open("w", encoding="utf-8") as handle:
            ...
            with lock:
                state["jobs"][key] = dict(
                    status="running", pid=process.pid, command=cmd, log=str(logfile), started=time.time()
                )
                write_json(state_path, state)
        ...
        if code:
            raise RuntimeError(key + " failed; see " + str(logfile))
                ...
                for name, seed in bundles[gpu::devices]:
                    ...
                    launch(
                        ...
                        [
                            ...
                            c.NEW_BASELINE_DATA,
                            "--steps",
                            c.NEW_BASELINE_STEPS,
                            "--batch-size",
                            4,
                            "--eval-every",
                            c.NEW_BASELINE_STEPS // 5,
                            "--seed",
                            seed,
                            "--device",
                            device,
                    ...
                    launch(
                        ...
                        [
                            ...
                            name,
                            "--candidates",
                            raw.with_suffix(".candidates"),
                            "--device",
                            device,
                            "--output",
                            c.CHECKPOINT_DIR / (name + "_policy_seed" + str(seed) + ".pt"),
                            "--num-agents",
                            c.NUM_AGENTS,
                            "--max-steps",
                            c.MAX_STEPS,
            ...
            launch(
                ...
                [
                    ...
                    *c.BENCH_MODELS,
                    ...
                    c.CHECKPOINT_DIR,
                    "--residual-checkpoint",
                    c.CHECKPOINT_DIR / "residual_policy.pt",
                    "--train-seeds",
                    *c.SEEDS,
                    "--require-matched-protocol",
                    "--n-boot",
                    c.N_BOOT,
                    "--output-dir",
                    c.RESULT_DIR,
            ...
            launch(
                ...
                [
                    ...
                    *c.SEEDS,
                    ...
                    c.N_BOOT,
                    "--output-dir",
                    c.RESULT_DIR / "param_lookahead_stress",
                    "--no-figures",
                ],
            )
            launch(
