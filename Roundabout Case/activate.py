"""Point the existing RL/benchmark stack at the Jounieh roundabout without editing repo files.

Geometry is the extracted Jounieh curb (outer ring minus islands), the same
polygon used in roundabout calibration. Decision, local observations, OBB
checks, and the bicycle grid stay on the highway protocol.
"""
...
def apply() -> None:
    ...
    from network import load_site_corridor, road_region_from_site, site_vehicle_size
    ...
    vehicle_length, vehicle_width = site_vehicle_size()
    import hashlib
    ...
    site_protocol = dict(
        ...
        test_seed_block=[810000, 820000],
        hashes={p.relative_to(REPO_ROOT).as_posix(): hashlib.sha256(p.read_bytes()).hexdigest() for p in source_paths},
    )

    def load_site(*_args, **_kwargs):
        return site

    corridor.load_corridor = load_site
    traffic_env.load_corridor = load_site
    boundary.load_corridor = load_site
    import RL.behavior_reference as behavior_reference
    import Baselines.scenario as scenario_mod

    behavior_reference.load_corridor = load_site
    scenario_mod.load_corridor = load_site
    ...
    def site_road_region(corridor_obj, margin=0.0):
    ...
    boundary.road_region = site_road_region

    original_post = traffic_env.EnvConfig.__post_init__

    def site_post_init(self):
        ...
        sim["wheelbase"] = float(VEHICLE_WHEELBASE)
        sim["destination_threshold"] = DESTINATION_THRESHOLD
        sim["max_agent_speed"] = float(MAX_AGENT_SPEED)
    ...
    traffic_env.EnvConfig.__post_init__ = site_post_init

    from recorded_spawns import RecordedSpawnPool, split_for_seed

    # Keep partitions separate while preserving observed simultaneous poses.
    ...
    def site_path_adherence(candidate_pos, nominal_y, params, sim_config=None):
    ...
    utility_model.path_adherence_penalty = site_path_adherence
    ...
    def site_dir(current_pos, candidate_pos, destination_pos, current_heading_vector, params):
        sim = _tls.get("sim")
        on_road = None if sim is None else sim.get("_on_road_dest")
        if on_road is None:
            return original_dir(current_pos, candidate_pos, destination_pos, current_heading_vector, params)
    ...
    def site_dist(candidate_pos, destination_pos, current_speed, params, sim_config):
        on_road = sim_config.get("_on_road_dest") if sim_config is not None else None
        if on_road is None:
            return original_dist(candidate_pos, destination_pos, current_speed, params, sim_config)
    ...
    utility_model.evaluate_candidate_utility = site_evaluate
    utility_model.directional_alignment_utility = site_dir
    utility_model.distance_reward_utility = site_dist
